...
*/
#ifndef P12218319_ANN_NETWORK4_INL
#define P12218319_ANN_NETWORK4_INL
...
namespace P12218319 { namespace ann {
	...
	class Network<LAYER_COUNT_,  NEURON_COUNTS, typename std::enable_if<LAYER_COUNT_ == 4>::type> : public NetworkI{
	public:
		enum {
			LAYER_COUNT = LAYER_COUNT_
			INPUT_COUNT = NEURON_COUNTS[0],
			OUTPUT_COUNT = NEURON_COUNTS[3]
		};
	private:
		typedef Layer<NEURON_COUNTS[1], INPUT_COUNT>> LayerType0;
		typedef Layer<NEURON_COUNTS[2], NEURON_COUNTS[1]>> LayerType1;
		typedef Layer<OUTPUT_COUNT, NEURON_COUNTS[2]>> LayerType2;
	...
	public:
		...
		void P12218319_CALL operator()(const float* const aInputs, float* const aOutputs) const throw() override {
			...
			float layerBuffer1[LayerType1::OUTPUT_COUNT];
			
			mLayer0(aInputs, layerBuffer0);
			mLayer1(layerBuffer0, layerBuffer1);
			mLayer2(layerBuffer1, aOutputs);
		}
	};
}}
